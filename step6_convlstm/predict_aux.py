...
from step6_convlstm.train import setup_auxiliary_data_dirs
from step6_convlstm.model import XCO2ConvLSTM

def parse_args():
    """解析命令行参数。"""
    parser = argparse.ArgumentParser(description='带辅助特征的XCO2预测脚本')
    ...
    # 输出参数
    parser.add_argument('--output_dir', type=str, default=r'./2018预测结果_带辅助特征',
                        help='保存预测TIF的目录')
...
def main():
    ...
    # 获取辅助数据目录
    auxiliary_dirs = setup_auxiliary_data_dirs()
    print(f"已加载 {len(auxiliary_dirs)} 个辅助特征数据目录")
    
    # 初始化和加载模型
    model = XCO2ConvLSTM(
        input_channels=9,  # 使用正确的输入通道数，根据你的模型调整
        hidden_dims=[32, 64],
        kernel_size=3,
        num_layers=2,
        dropout=0.2
    ...
    for file in input_files:
        ...
        # 加载辅助特征
        aux_data = create_auxiliary_input(file, auxiliary_dirs)
        
        # 将XCO2数据添加为第一个通道
        xco2_with_channel = data[np.newaxis, :, :]
        
        # 合并XCO2和辅助特征
        combined_data = np.concatenate([xco2_with_channel, aux_data], axis=0)
    ...
    # 从加载的数据创建序列张量
    input_sequence = np.stack(input_data)  # [seq_len, channels, height, width]
    ...
    with torch.no_grad():
        for step in range(args.num_steps):
            ...
            # 为下一次预测准备输入：获取最后一步的辅助特征
            pred_date = start_date + relativedelta(months=step+1)
            
            # 创建新的时间步
            new_step = torch.zeros_like(current_input[:, 0:1])
            ...
            # 获取对应日期的辅助特征（如果有可能的话）
            # 这里假设辅助特征可以用最新的代替
            # 如果需要精确的辅助特征，可能需要额外的处理
            for i in range(1, new_step.shape[2]):
                new_step[0, 0, i] = current_input[0, -1, i]
    ...
    for i, (pred, mask) in enumerate(zip(predictions, masks)):
        ...
        # 保存为TIF
        out_path = os.path.join(args.output_dir, f"{date_str}_CONVLSTM_AUX_XCO2.tif")
        profile.update({
            'count': 1,
            'dtype': 'float32',
            'compress': 'lzw',
    ...
    if args.visualize:
        ...
        for i, (pred, mask) in enumerate(zip(predictions, masks)):
            ...
            plt.title(f'{date_str} 的 XCO2 预测 (带辅助特征)')
            plt.tight_layout()
            
            plt.savefig(os.path.join(viz_dir, f'prediction_aux_{date_str}.png'), dpi=300)
            plt.close()
